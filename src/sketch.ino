...
//set the pin
/////////////////////////////////////////////////////////////////////
const int enableTransmitPin = 10;
...
void setup() {                
 ...
 // start the SPI library:
  ...
  pinMode(enableTransmitPin,OUTPUT);             // Set digital pin 10 as OUTPUT to connect it to the RFID /ENABLE pin 
    digitalWrite(enableTransmitPin, LOW);          // Activate the RFID reader
...
/*
 * Write a byte of data to a certain MFRC522 register 
 *
 * Input Parameters：
...
{
  digitalWrite(enableTransmitPin, LOW);

  // Address Format：0XXXXXX0
  SPI.transfer((addr << 1) & 0x7E);  
  SPI.transfer(val);
  
  digitalWrite(enableTransmitPin, HIGH);
}


/*
 * Read a byte of data from a certain MFRC522 register
 *
 * Input Parameters: 
 *    addr: register address
 *
 * Returns: a byte of data read from the given MFRC522 register
 */
...
/*
 * Set RC522 register bits form a bitmask
 *
 * Input parameters: 
...
/*
 * Clear RC522 register bits form a bitmask
 *
 * Input parameters: 
 *    reg:  register address; 
 *    mask: bits to clear
...
/*
 * Open antennas, each time you start or shut down, 
 * the natural barrier between the transmitter should 
...
/*
 * Close antennas, each time you start or shut down, 
 * the natural barrier between the transmitter should 
 * be at least 1ms interval
 */
 ...
 *       sendData: the data send to the card
 *       sendLen:  Length of data to sent   
...
uchar MFRC522_ToCard(uchar command, 
  ...
  i = 2000;  // i according to the clock frequency adjustment, 
            // the operator M1 card maximum waiting time is 25ms
  do {
    // CommIrqReg[7..0]
 ...
 *
 * Input parameters: 
 *    serNum: 5 byte checksum + 4 bytes chard serial number
 *
 ...
 * Input parameters: 
 *    pIndata:  data to calculate the CRC for, 
 *    len:      the data length, 
 *    pOutData: CRC calculation result
 */
 ...
 *
 * Return value: on success, the card capacity
 */
 ...
 * Return value: the successful return MI_OK
 */
uchar MFRC522_Auth(uchar authMode, 
                   uchar BlockAddr, 
                   uchar *Sectorkey, 
                   uchar *serNum) {
 ...
 * Input parameters: 
 *    blockAddr: block address; 
 *    writeData: 16-byte data block to write
 *
 * Return value: on success MI_OK
 */
uchar MFRC522_Write(uchar blockAddr, uchar *writeData)
{
  ...
  if (status == MI_OK)
  {
    for (i = 0; i < 16; i++)    // copy data to the buffer
      buff[i] = writeData[i];   
